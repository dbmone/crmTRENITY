...
import { LogOut, Bell, X, Settings, LayoutDashboard, Archive, Users } from "lucide-react";
...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-bg-surface border-b border-bg-border flex items-center justify-between px-6 py-3 gap-4">
      {/* Logo */}
      ...
      <nav className="hidden md:flex items-center gap-1">
        {NAV.filter((n) => !n.adminOnly || canSeeAdmin).map((item) => {
      ...
      {user && (
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <div className="relative" ref={notifRef}>
            ...
            {showNotifs && (
              <div className="absolute right-0 top-full mt-2 w-80 bg-bg-surface border border-bg-border rounded-modal shadow-modal overflow-hidden animate-modal">
          ...
          >
            ...
            <span className="hidden sm:block text-sm font-medium text-ink-primary">{user.displayName}</span>
          </button>

          {/* Logout */}
          <button
            onClick={() => { logout(); navigate("/login"); }}
            className="p-2 rounded-lg hover:bg-bg-hover transition-colors text-ink-tertiary hover:text-red-400"
            title="Выйти"
          >
            <LogOut size={16} />
          </button>
