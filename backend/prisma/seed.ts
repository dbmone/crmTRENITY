import { PrismaClient, UserRole, StageName } from "@prisma/client";

const prisma = new PrismaClient();

async function main() {
  console.log("🌱 Seeding database...");

  // Тестовые пользователи
  const anna = await prisma.user.upsert({
    where: { telegramId: BigInt(100001) },
    update: {},
    create: {
      telegramId: BigInt(100001),
      telegramUsername: "anna_mk",
      displayName: "Анна Маркетолог",
      role: UserRole.MARKETER,
      pinCode: "A1b2",
      chatId: BigInt(100001),
    },
  });

  const sergey = await prisma.user.upsert({
    where: { telegramId: BigInt(100002) },
    update: {},
    create: {
      telegramId: BigInt(100002),
      telegramUsername: "sergey_pm",
      displayName: "Сергей PM",
      role: UserRole.MARKETER,
      pinCode: "C3d4",
      chatId: BigInt(100002),
    },
  });

  const dima = await prisma.user.upsert({
    where: { telegramId: BigInt(100003) },
    update: {},
    create: {
      telegramId: BigInt(100003),
      telegramUsername: "dima_video",
      displayName: "Дима Видеомейкер",
      role: UserRole.LEAD_CREATOR,
      pinCode: "E5f6",
      chatId: BigInt(100003),
    },
  });

  const kate = await prisma.user.upsert({
    where: { telegramId: BigInt(100004) },
    update: {},
    create: {
      telegramId: BigInt(100004),
      telegramUsername: "kate_edit",
      displayName: "Катя Монтажёр",
      role: UserRole.CREATOR,
      pinCode: "G7h8",
      chatId: BigInt(100004),
    },
  });

  const max = await prisma.user.upsert({
    where: { telegramId: BigInt(100005) },
    update: {},
    create: {
      telegramId: BigInt(100005),
      telegramUsername: "max_cr",
      displayName: "Макс Креатор",
      role: UserRole.CREATOR,
      pinCode: "J9k0",
      chatId: BigInt(100005),
    },
  });

  console.log("✅ Users created");

  // Тестовый заказ
  const order1 = await prisma.order.create({
    data: {
      title: "Рилс для Ozon — весенняя коллекция",
      description:
        "Сделать AI-видео для весенней коллекции одежды. Стиль: минимализм, пастельные тона. Хронометраж 30 сек.",
      status: "IN_PROGRESS",
      deadline: new Date(Date.now() + 5 * 24 * 60 * 60 * 1000), // +5 дней
      reminderDays: 2,
      marketerId: anna.id,
      creators: {
        create: [
          { creatorId: dima.id, addedById: anna.id, isLead: true },
          { creatorId: kate.id, addedById: anna.id, isLead: false },
        ],
      },
      stages: {
        create: [
          { name: StageName.STORYBOARD, status: "DONE", sortOrder: 1, startedAt: new Date(), completedAt: new Date() },
          { name: StageName.ANIMATION, status: "IN_PROGRESS", sortOrder: 2, startedAt: new Date() },
          { name: StageName.EDITING, status: "PENDING", sortOrder: 3 },
          { name: StageName.REVIEW, status: "PENDING", sortOrder: 4 },
          { name: StageName.COMPLETED, status: "PENDING", sortOrder: 5 },
        ],
      },
    },
  });

  const order2 = await prisma.order.create({
    data: {
      title: "Обзор нового продукта для YouTube",
      description: "AI-обзор нового гаджета. Нужна раскадровка + анимация + монтаж.",
      status: "NEW",
      deadline: new Date(Date.now() + 10 * 24 * 60 * 60 * 1000), // +10 дней
      reminderDays: 3,
      marketerId: sergey.id,
      stages: {
        create: [
          { name: StageName.STORYBOARD, status: "PENDING", sortOrder: 1 },
          { name: StageName.ANIMATION, status: "PENDING", sortOrder: 2 },
          { name: StageName.EDITING, status: "PENDING", sortOrder: 3 },
          { name: StageName.REVIEW, status: "PENDING", sortOrder: 4 },
          { name: StageName.COMPLETED, status: "PENDING", sortOrder: 5 },
        ],
      },
    },
  });

  const order3 = await prisma.order.create({
    data: {
      title: "Промо-ролик для VK Clips",
      description: "Короткий AI-ролик 15 сек. Динамичный монтаж.",
      status: "ON_REVIEW",
      deadline: new Date(Date.now() + 1 * 24 * 60 * 60 * 1000), // +1 день
      reminderDays: 1,
      marketerId: anna.id,
      creators: {
        create: [{ creatorId: max.id, addedById: anna.id, isLead: false }],
      },
      stages: {
        create: [
          { name: StageName.STORYBOARD, status: "DONE", sortOrder: 1, startedAt: new Date(), completedAt: new Date() },
          { name: StageName.ANIMATION, status: "DONE", sortOrder: 2, startedAt: new Date(), completedAt: new Date() },
          { name: StageName.EDITING, status: "DONE", sortOrder: 3, startedAt: new Date(), completedAt: new Date() },
          { name: StageName.REVIEW, status: "IN_PROGRESS", sortOrder: 4, startedAt: new Date() },
          { name: StageName.COMPLETED, status: "PENDING", sortOrder: 5 },
        ],
      },
    },
  });

  console.log("✅ Orders created");
  console.log("");
  console.log("🎉 Seed complete!");
  console.log("");
  console.log("Тестовые PIN-коды:");
  console.log("  Анна (маркетолог):      A1b2");
  console.log("  Сергей (маркетолог):     C3d4");
  console.log("  Дима (лид-креатор):      E5f6");
  console.log("  Катя (креатор):          G7h8");
  console.log("  Макс (креатор):          J9k0");
}

main()
  .catch((e) => {
    console.error("❌ Seed error:", e);
    process.exit(1);
  })
  .finally(async () => {
    await prisma.$disconnect();
  });
